...
`mcrctl doctor --project <path>`はcurrent lockとcanonical generated bytesをDocker接続前に照合し、
local Unix Docker context、managed volume、current container、running / healthy、
lockどおりのpublish portを確認する。さらにCompose labelから、running containerがcanonical
generated `compose.yaml`とworking directoryだけで作られたかを確認する。追加Compose fileを
使ったruntimeはhealth / protocol検査を続けるが、canonicalと同一視せず
`render=additional-compose-files`を警告する。未知のCompose provenanceは`render=noncanonical`とする。
既定outputは`<project>/generated`、既定contextは`default`で、必要な場合だけ明示overrideする。
