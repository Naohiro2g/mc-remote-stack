...
失敗時はDocker commandの終了statusに加え、stderr（空ならstdout）の最後の非空行だけを
制御文字除去・秘密値mask・長さ制限後にdetailとして表示できる。Docker stderr全体やcontainer
logは通常のCLI出力へ転記しない。秘密を含む調査logが必要ならGit外へ置く。
...
- current lockとcanonical generated bytesをDocker接続前に検証する。
- local Unix Docker context、daemon、Compose configをread-only確認する。
- current container、managed volume、running / healthy、exact port publishを確認する。
- Compose labelからrunning containerのconfig fileとworking directoryを検証する。canonical
  generated `compose.yaml`以外も使われていればhealth / protocol検査は続けるが、
...
- 同じlockでも追加Compose fileを使うruntimeはno-opにせず、mutation前に拒否する
